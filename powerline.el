...
(defface powerline-active1 '((t (:background "#0D1011" :inherit mode-line)))
  "Powerline face 1."
  :group 'powerline)

(defface powerline-active2 '((t (:background "#55B5DB" :foreground "black" :inherit mode-line))) ;;#8BE03C
  "Powerline face 2."
  :group 'powerline)
...
(defun ~powerline-github-vc ()
  (let ((branch (mapconcat 'concat (cdr (split-string vc-mode "[:-]")) "-")))
    (concat
      ;; (propertize (format " %s" (all-the-icons-alltheicon "git")) 'face 'all-the-icons-lblue)
      (propertize (all-the-icons-faicon "git")
        'face `(:family ,(all-the-icons-octicon-family) :height 0.9)
        'display '(raise -0.1))
      " ⸱ "
      (propertize (all-the-icons-octicon "git-branch")
        'face `(:family ,(all-the-icons-octicon-family) :height 0.9 :foreground "#8BE03C")
        'display '(raise 0.1))
...
(defun codefalling//simplify-major-mode-name ()
  ...
  (let* ((major-name (format-mode-line mode-name))
...
;;;###autoload
(defpowerline powerline-tspyimt-major-mode
...
(eval-after-load 'evil
  '(progn
     (defface powerline-evil-insert-face
       '((((class color))
           :foreground "black" :background "#8BE03C" :weight bold :inherit mode-line)
          (t (:weight bold)))
     ...
     (defface powerline-evil-visual-face
       '((((class color))
           :foreground "white" :background "#eab700" :weight bold :inherit mode-line)
          (t (:weight bold)))
     ...
     (defface powerline-evil-emacs-face
       '((((class color))
           :foreground "white" :background "blue violet" :weight bold :inherit mode-line :height 1.0)
          (t (:weight bold)))
     ...
     (setq
       ...
       (propertize (all-the-icons-fileicon "emacs")
         'face 'powerline-evil-emacs-face 'display '(raise -0.1)))
...
(defun powerline-icon-vc ()
  (when vc-mode
    (cond
      ((string-match "Git[:-]" vc-mode) (~powerline-github-vc))
      (t (format "%s" vc-mode)))))

;;;###autoload
(defpowerline powerline-proj
  (concat
    (propertize (all-the-icons-fileicon "premiere")
      'face `(:family ,(all-the-icons-fileicon-family) :height 0.9 :foreground "#eab700")
      'display '(raise -0.1)
      )
    (propertize
      (concat
  ...
  (cond
    (buffer-read-only
      (propertize (all-the-icons-octicon "lock")
        'face `(:family ,(all-the-icons-octicon-family) :height 0.8 :foreground "#c82829")
        'display '(raise 0.1)
        ))
    ((buffer-modified-p)
      (propertize (all-the-icons-faicon "chain-broken")
        'face `(:family ,(all-the-icons-faicon-family) :height 0.8 :foreground "#eab700")
        'display '(raise 0.1)
        ))
    (t (propertize (all-the-icons-faicon "link")
         'face `(:family ,(all-the-icons-faicon-family) :height 0.8 :foreground "#9FCA56")
         'display '(raise 0.1)
         )))
  )
...
(defpowerline powerline-vc
  (when (and
    ...
    (if (and
          window-system
          (not powerline-gui-use-vcs-glyph))
  ...
  (powerline-raw
    (format-mode-line
      (concat
        ...
        (propertize
          "%b"
          'face '(:foreground "#9FCA56" :weight bold)
          'mouse-face 'mode-line-highlight
